...
module MiniTest
  ...
  file = if RUBY_VERSION =~ /^1\.9/ then  # bt's expanded, but __FILE__ isn't :(
           ...
           require 'pathname'
           # Pathname#relative_path_from does not deal with the the drive letter using different cases
           # This is a workaround for http://redmine.ruby-lang.org/issues/show/1266
           if RUBY_PLATFORM =~ /mswin|mingw/
             pwd = Pathname.new Dir.pwd.downcase
             pn = Pathname.new File.expand_path(__FILE__).downcase
           else
