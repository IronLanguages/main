...
namespace Microsoft.IronRubyTools.Editor.Core {
    /// <summary>
    /// Provides highlighting of matching braces in a text view.
    /// </summary>
    public class BraceMatcher {
        private readonly ITextView _textView;
        private SimpleTagger<TextMarkerTag> _textMarkerTagger;
        ...
        private void RemoveExistingHighlights() {
            if (_hasSpan && _textMarkerTagger != null) {
                TextMarker.RemoveTagSpans(x => true);
                _hasSpan = false;
            }
        }

        private SimpleTagger<TextMarkerTag> TextMarker {
            get {
                if (_textMarkerTagger == null) {
                    _textMarkerTagger = _compModel.GetService<ITextMarkerProviderFactory>().GetTextMarkerTagger(_textView.TextBuffer);
                }
                return _textMarkerTagger;
            }
        }
        ...
        private static bool IsOpenSpan(IList<VisualStudio.Text.Classification.ClassificationSpan> spans, int i) {
            return spans[i].ClassificationType == RubyClassifierProvider.Instance.OpenGroupingClassification ||
        ...
        private static bool IsCloseSpan(IList<VisualStudio.Text.Classification.ClassificationSpan> spans, int i) {
            return spans[i].ClassificationType == RubyClassifierProvider.Instance.CloseGroupingClassification ||
                (spans[i].ClassificationType.IsOfType(PredefinedClassificationTypeNames.Operator) &&
                spans[i].Span.Length == 1 &&
                (spans[i].Span.GetText() == "}" || spans[i].Span.GetText() == "]"));
        }
