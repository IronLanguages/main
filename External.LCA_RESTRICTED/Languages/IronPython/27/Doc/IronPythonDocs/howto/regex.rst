...
``|``
   ...
   multi-character strings. ``Row|Servo`` will match either ``Row`` or ``Servo``,
   not ``Ro``, a ``'w'`` or an ``'S'``, and ``ervo``.
...
``Row`` must match starting with a ``'R'``.  The analysis lets the engine
quickly scan through the string looking for the starting character, only trying
the full match if a ``'R'`` is found.
