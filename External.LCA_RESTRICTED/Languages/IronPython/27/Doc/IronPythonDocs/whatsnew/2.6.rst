...
This article explains the new features in IronPython 2.6.

The major theme of IronPython 2.6 is preparing the migration path to
Python 3.0, a major redesign of the language.  Whenever possible,
IronPython 2.6 incorporates new features and syntax from 3.0 while
remaining compatible with existing code by not removing older features
or syntax.  When it's not possible to do that, IronPython 2.6 tries to do
...
IronPython 2.6 also sees a number of improvements and bugfixes throughout
...
full details, you should refer to the documentation for IronPython 2.6. If
...
such as :func:`bin` have simply been added to IronPython 2.6, but existing
...
The fix for IronPython 2.6 adds a :attr:`__package__` attribute to
...
IronPython 2.6 introduces a convention for user-specific site directories.
...
has been backported to IronPython 2.6.
...
IronPython 2.6 has a ``__future__`` import that removes ``print`` as language
...
the first example as catching two different exceptions.  IronPython 2.6
...
IronPython 2.6 adds :class:`bytes` as a synonym for the :class:`str` type,
...
to :ctype:`PyBytesObject`.  IronPython 2.6 uses ``#define``
...
* :class:`RawIOBase` defines raw I/O operations: :meth:`read`,
  ...
  sockets, but IronPython 2.6 hasn't restructured its file and socket objects
  in this way.
...
* :class:`TextIOBase`: Provides functions for reading and writing
  ...
  (In IronPython 2.6, :class:`io.StringIO` is implemented in
...
In IronPython 2.6, the underlying implementations haven't been
...
The IronPython 2.6 :mod:`collections` module includes a number of
...
IronPython 2.6 doesn't drop support for a leading 0 signalling
...
converted to floats.  IronPython 2.6 adds a simple rational-number class,
...
one, :func:`math.trunc`, that's been backported to IronPython 2.6.
...
* The :func:`hasattr` function was catching and ignoring all errors,
  ...
  :exc:`KeyboardInterrupt` and :exc:`SystemExit`, however; IronPython 2.6
...
* Instance method objects have new attributes for the object and function
  ...
  The old names are still supported in IronPython 2.6, but are gone in 3.0.
...
* Type objects now have a cache of methods that can reduce
  ...
  IronPython 2.6 by Kevin Jacobs; :issue:`1700288`.)
...
Jython, or IronPython.  If either option is used with IronPython 2.6, the
...
* The :mod:`itertools` module gained several new functions.
  ...
  :mod:`itertools` that gained a new constructor in IronPython 2.6.
...
* The :mod:`random` module's :class:`Random` objects can
  ...
  that IronPython 2.6's :class:`Random` objects can't be unpickled correctly
...
* Long regular expression searches carried out by the  :mod:`re`
  ...
  so IronPython 2.6 includes a verifier for the regex bytecode.
...
Bill Janssen made extensive improvements to IronPython 2.6's support for
...
* :meth:`object.__init__` previously accepted arbitrary arguments and
  keyword arguments, ignoring them.  In IronPython 2.6, this is no longer
