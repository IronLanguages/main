...
This article explains the new features in IronPython 2.0.  

The changes in IronPython 2.0 are an interesting mix of language and library
...
details, you should always refer to the documentation for IronPython 2.0 at
...
The more complicated part has been implemented in IronPython 2.0: importing a module
...
In IronPython 2.0, you can switch :keyword:`import`'s behaviour to  absolute imports
...
Until IronPython 2.0, the :keyword:`try` statement came in two flavours. You could
...
and this clarified what the statement should mean.  In IronPython 2.0, you can now
write::
...
IronPython 2.0 adds a simple way to pass values *into* a generator. As introduced in
...
To enable the statement in IronPython 2.0, you need to add the following directive
to your module::
...
inheritance relationships are::
   ...
   BaseException       # New in IronPython 2.0
   |- KeyboardInterrupt
   |- SystemExit
   |- Exception
...
In IronPython 2.0, you can now write ``except Exception`` to achieve the same
...
occurred"``, is deprecated in IronPython 2.0 and will trigger a warning.  The aim is
...
Here are all of the changes that IronPython 2.0 makes to the core Python language.
...
* One error that Python programmers sometimes make is forgetting to include an
  ...
  log all the paths searched. In IronPython 2.0, a new :exc:`ImportWarning` warning is
...
In IronPython 2.0, ``quit`` and ``exit`` are now objects that still produce string
...
* Python's built-in exceptions are now new-style classes, a change that speeds
  up instantiation considerably.  Exception handling in IronPython 2.0 is therefore
...
The standard library received many enhancements and bug fixes in IronPython 2.0.
...
* Library: the :mod:`locale` module's  :func:`format` function's would
  ...
  appeared.  In IronPython 2.0, the argument must be exactly one %char specifier with
  no surrounding text.
