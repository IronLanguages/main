...
describe "Socket#getnameinfo" do
  ...
  it "gets the name information and doesn't resolve it" do
  ...
  it "gets the name information and resolves the host" do
    sockaddr = Socket.sockaddr_in SocketSpecs.port, '127.0.0.1'
    name_info = Socket.getnameinfo(sockaddr, Socket::NI_NUMERICSERV)
    name_info.should == [@hostname, "#{SocketSpecs.port}"]
  end
