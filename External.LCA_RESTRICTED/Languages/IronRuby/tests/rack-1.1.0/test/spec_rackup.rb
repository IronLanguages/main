...
# begin
# require "mongrel"
# jimmysch -- don't depend on Mongrel; let it fall back to WEBrick

# jimmysch -- add RUBY_FLAGS so rack.rb can be found
RUBY_FLAGS = '-I../../lib'
...
context "rackup" do
  ...
  def run_rackup(*args)
    ...
    flags   = args.first
    @host = options[:host] || 'localhost' #"0.0.0.0" # jimmysch -- for some reason 0.0.0.0 doesn't work 
    @port = options[:port] || 9292

    Dir.chdir("#{root}/test/rackup") do
      @in, @rackup, @err = Open3.popen3("#{Gem.ruby} #{RUBY_FLAGS} -S #{rackup} #{flags}")
    end
  ...
  # jimmysch -- enable retrying a set number of times ...
