...
class FormV7(Form):
    ...
    def OnMsgButtonClick(self, *args):
        try:
            mapHelper = MapPointWebServiceHelper.GetInstance("5200",
                                                             "ned68Fe")
            loc = mapHelper.FindLocation(self._txtLocation.Text)
            if loc:
                pb = self._pictureBox1
                pb.Image = mapHelper.GetMap(loc, pb.Width, pb.Height, 4.0)
            else:
                MessageBox.Show("Address or location is not valid",
                                "Invalid Location")
        except Exception, e:
